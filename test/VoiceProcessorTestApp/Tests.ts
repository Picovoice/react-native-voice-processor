...
async function getDetections(porcupine, audioFilePath) {
    ...
    for (let i = 0; i < pcm.length - frameLength; i += frameLength) {
        const detection = await porcupine.process(
            pcm.slice(i, i + frameLength),
        );
        if (detection !== -1) {
...
async function runTestcase(
    ...
    try {
        const keywordPaths = keywords.map(keyword =>
            getPath(`keyword_files/${language}/${keyword}_${platform}.ppn`),
        );
        const modelPath =
            language === 'en'
...
async function singleKeywordTest(testcases): Result[] {
    ...
    for (const testcase of testcases) {
        const audioFilePath = getPath(
            `audio_samples/${testcase.wakeword.replaceAll(' ', '_')}.wav`,
        );
